 ...
 NON-MEMBER functions for the statistician class:
   ...
   sequence operator +(const sequence& lhs, const sequence& rhs)
     ...
     numbers of the sequences of lsh and rhs. (LEFT ARRAY RIGHT ARAY)
...
Three member functions work together to enforce the in-order retrievalrule:
void start( );
value_type current( ) const;
void advance( );
§After activating start, the currentfunction returns the first item 
§Each time we call advance, the current function changes so that it returns the next item in the sequence

For a sequence x, we would like to be able to refer to the individual items using the usual C++ notation for arrays. 
For example, if xhas three items, then we want to be able  to  write x[0], x[1],and x[2]to  access  these  three  items.  
This  use  of  the square bracketsis called the subscript operator. The subscript operator may be 
...
More information about the sequence class is available in Slide Set 3
    ...
    Do we have to create a new array with size capacity? What do we declare as the data type?
    The variable type of your size is your size_type variable which you define yourself as (size_t), then you would  
    create a capacity variable of type size_type, and initialize it to whatever your max capacity is. --- so like : 
    (size_type CAPACITY = your capacity 
    The variable type of your array is your value_type variable which you define yourself as (whatever it is at the time - 
    I would set it to double), but the variable definition is convenient if you want to quickly change it. --- so like: 
    value_type data[CAPACITY]; 
    So all your variables that deal with data type of the array are value_type and all variables that deal with indices or sizes are 
    size_type --- ( you can also just use int for indices, this isn't as important) */
...
namespace coen79_lab3 {
    class sequence {
        private:
            static const size_t CAPACITY = 30; //idk man just shut up
                //sequence::CAPACITY is the maximum number of items that a sequence can hold.
        ...
        public:
            ...
            double mean() const;
            double standardDeviation() const;
            value_type operator[](int indx) const;
            void operator +=(const sequence& rhs);
    };
        sequence operator +(const sequence& lhs, const sequence& rhs);
